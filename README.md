...
| `--gnome-file PATH`   | explicit dconf path (default: alongside the snapshot)       |
...
| `udev`            | Local hardware-permission rules at `/etc/udev/rules.d/*.rules` — the bits that let you flash a QMK keyboard, talk to a PlatformIO microcontroller, or read an ST-Link / Saleae without sudo. Two filter passes drop noise: rules owned by apt packages (filtered via batched `dpkg -S` — they come back via `apt install`), and rules whose body identifies them as machine-generated (`# This file is automatically generated.` for snapd's `70-snap.*.rules`, `# Written by curtin` for the Ubuntu installer's LVM volume rules — those come back from `snap install` / a fresh Ubuntu install). What's left is the user-installed rules that won't come back automatically. The full rule body is base64-encoded into `payload:`. Restore writes each file back via `sudo install -m 644 -o root -g root` and runs a single `sudo udevadm control --reload-rules && sudo udevadm trigger` per batch. Idempotent: a target file with identical bytes is skipped. |
